...
from models_db import StrategyConfig, StrategyLock, User
from strategies.registry import get_registry, load_default_strategies
from core.signal_logger import log_signal
from core.evaluator import evaluate_pending_signals
from core.trial_policy import is_trial_active
from core.telegram_gate import can_access_telegram
...
def get_active_strategies_from_db() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for c in configs:
            ...
            if user and c.telegram_chat_id:
                if can_access_telegram(user):
...
class StrategyScheduler:
    ...
    def acquire_lock(self, db: Session) -> bool:
        now = datetime.utcnow()
        lock = db.query(StrategyLock).filter(StrategyLock.id == 1).first()

        if not lock:
            lock = StrategyLock(id=1, owner_id=self.lock_id, expires_at=now + timedelta(seconds=self.lock_ttl))
            db.add(lock)
            db.commit()
            return True
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            LOG.info("HEARTBEAT iter=%s personas=%s", iteration, len(personas))
