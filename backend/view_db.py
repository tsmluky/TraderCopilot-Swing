...
def main():
    ...
    try:
        ...
        if total_signals > 0:
            ...
            print("\n🔍 Señales por Modo:")
            modes = (
                db.query(Signal.mode, func.count(Signal.id)).group_by(Signal.mode).all()
            )
            ...
            print("\n💰 Señales por Token:")
            tokens = (
                db.query(Signal.token, func.count(Signal.id))
                .group_by(Signal.token)
                .all()
            )
            ...
            print("-" * 80)
            recent_signals = (
                db.query(Signal).order_by(Signal.timestamp.desc()).limit(10).all()
            )
            for sig in recent_signals:
                print(
                    f"  {sig.timestamp} | {sig.token:6} | {sig.mode:8} | "
                    f"{sig.direction:5} | Entry: ${sig.entry:,.2f}"
                )
        ...
        if total_evals > 0:
            ...
            results = (
                db.query(SignalEvaluation.result, func.count(SignalEvaluation.id))
                .group_by(SignalEvaluation.result)
                .all()
            )
            ...
            for ev in recent_evals:
                sig = ev.signal
                print(
                    f"  {ev.evaluated_at} | {sig.token:6} | {ev.result:8} | "
                    f"Exit: ${ev.exit_price:,.2f}"
                )
