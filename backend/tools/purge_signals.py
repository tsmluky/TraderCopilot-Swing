...
def purge_signals(strategy_id=None, execute=False):
    ...
    try:
        if strategy_id:
            ...
            # Delete ALL
            print(
                "Targeting ALL signals (and evaluations via cascade if configured)..."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--strategy", help="Strategy ID to purge (e.g., '1234' or 'rsi_divergence_v1')"
    )
    parser.add_argument("--all", action="store_true", help="Delete ALL signals")
    parser.add_argument(
        "--force", action="store_true", help="Actually execute the deletion"
    )
