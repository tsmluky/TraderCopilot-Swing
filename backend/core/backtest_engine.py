...
class BacktestEngine:
    ...
    def load_strategy(self, strategy_id: str):
        """
        Carga una estrategia por su ID.
        Prioridad:
        1. Registry (Memoria/Oficial)
        2. Import dinámico por nombre de archivo (Legacy)
        """
        try:
            ...
            strategy_instance = registry.get(strategy_id)

            if strategy_instance:
                self.strategies[strategy_id] = strategy_instance
                print(f"[Backtest] Estrategia cargada desde Registry: {strategy_id}")
                return

            # 2. Fallback: Dynamic Import (Legacy filename match)
            print(
                f"[Backtest] Strategy '{strategy_id}' not in registry. Trying file import..."
            )
            ...
            if found_class:
                ...
                print(f"[Backtest] Estrategia cargada desde archivo: {strategy_id}")
            else:
    ...
    def run(
        self, strategy_id: str, symbol: str, timeframe: str = "1h", days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        try:
                        ...
                        trades.append(
                            {
                                ...
                                "exit_ts": self._safe_float(
                                    current_ts_val
                                ),  # Add for chart markers
                                "symbol": symbol.upper(),
                    ...
                    equity_curve.append(
                        {
                            ...
                            "strategy_equity": round(
                                self._safe_float(floating_equity), 2
                            ),
                            "buy_hold_equity": round(
                                self._safe_float(buy_hold_equity), 2
                            ),
            ...
            final_buy_hold = (
                equity_curve[-1]["buy_hold_equity"]
                if equity_curve
            ...
            return {
                "metrics": {
                    ...
                    "total_pnl": round(
                        self._safe_float(current_capital - self.initial_capital), 2
                    ),
                    "roi_pct": round(self._safe_float(roi_pct), 2),
                    "buy_hold_pnl": round(
                        self._safe_float(final_buy_hold - self.initial_capital), 2
                    ),
                    ...
                    "worst_trade": round(
                        self._safe_float(
                            min([t["pnl"] for t in trades]) if trades else 0
                        ),
                        2,
                    ),
                },
