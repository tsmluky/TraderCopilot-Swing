...
from core.timeframe_policy import normalize_timeframe
...
class LiteReq(BaseModel):
    ...
    def validate_timeframe(cls, v):
        ...
        # Accept legacy CCXT-friendly formats (lowercase) and normalize them.
        legacy = {"5m", "15m", "30m", "1h", "4h", "1d", "1w"}
        if raw.lower() in legacy:
            return raw.lower()

        # Accept Swing UI formats (1H/4H/1D) and normalize them.
        norm = normalize_timeframe(raw)
        allowed = {"1H", "4H", "1D"}
        if norm in allowed:
            return norm

        raise ValueError(f"Timeframe must be one of {sorted(list(legacy | allowed))}")
...
class ProReq(BaseModel):
    ...
    language: Optional[str] = Field(
        "es", description="Idioma del reporte: 'es' (Español) o 'en' (Inglés)."
    )
...
class LiteSignal(BaseModel):
    ...
    indicators: Optional[Dict[str, Any]] = Field(
        default=None, description="Snapshot de indicadores y estrategias para debugging/UI"
    )
