...
def verify_counts():
    ...
    try:
        ...
        cursor.execute(
            ...
            f"WHERE timestamp >= '{day_ago}' AND mode='LITE'"
        )
        ...
        cursor.execute(
            f"SELECT COUNT(*) FROM signals "
            f"WHERE timestamp >= '{day_ago}' AND mode='LITE' AND is_saved=1"
        )
        ...
        cursor.execute(
            "SELECT id, token, mode, is_saved, user_id FROM signals ORDER BY timestamp DESC LIMIT 5"
        )
        print("\nRecent 5 Signals:")
        for row in cursor.fetchall():
            print(f"  {row}")
