...
async def startup_event():
    ...
    # Start WatchAlert Checker (Background)
    from services.alerts_checker import check_watch_alerts
    async def alerts_loop():
        print("[BACKGROUND] Starting WatchAlerts Checker...")
        while True:
            try:
                # Run every 60s for now (MVP)
                await asyncio.sleep(60)
                await check_watch_alerts()
            except Exception as e:
                print(f"[BACKGROUND] Alert loop error: {e}")
                await asyncio.sleep(60)
