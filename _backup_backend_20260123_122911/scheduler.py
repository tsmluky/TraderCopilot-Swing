...
from core.entitlements import can_access_telegram  # noqa: E402
...
def get_active_strategies_from_db():
    ...
    db = SessionLocal()
    try:
        # Join with User to get Telegram preferences AND Plan info for Gating
        results = (
            db.query(
                StrategyConfig, 
        ...
        strategies = []
        for c, chat_id, plan, expires_at, role in results:
            # Gating Logic: Check if user can access Telegram
            # Construct a minimal User object for the policy check
            # (Or we could pass dicts if we refactor policy, but User object is expected)
            effective_chat_id = chat_id
            
            if chat_id:
                # Mock user for policy check to avoid full DB fetch/attach
                # We only need plan, plan_expires_at, role
                minimal_user = User(
                    id=c.user_id, 
                    plan=plan or "FREE", 
                ...
                if not can_access_telegram(minimal_user):
                     effective_chat_id = None
                     # Optional: Log that we suppressed notification for Trial user?
            ...
            # --- TOKEN LIST LOGIC (STRICT) ---
            # [HARDENING] 'ALL'/'SCANNER' expansion removed for Swing Pivot stability.
            # We strictly respect the tokens_list in DB or default to BTC.
...
class StrategyScheduler:
    ...
    def __init__(self, loop_interval: int = 60):
        ...
        # Registrar estrategias (Via Registry Centralizado)
        print("\n [INFO] Syncing with Strategy Registry...")
        # Ensure registry is loaded with defaults
        from strategies.registry import load_default_strategies
        load_default_strategies()
        
        print(" [INFO] Strategies synced.")
