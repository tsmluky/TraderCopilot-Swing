...
async def login_for_access_token(
    ...
    allowed_tokens = TokenCatalog.get_allowed_tokens(get_access_tier(user))
    ...
    return {
        ...
        "user": {
            ...
            "plan": plan_upper,
            "plan_status": getattr(user, "plan_status", "active"),
...
async def get_me(current_user = Depends(get_current_user)):
    ...
    return {
        ...
        "plan_status": ("expired" if (getattr(current_user, "plan_expires_at", None) is not None and getattr(current_user, "plan_expires_at") <= datetime.utcnow()) else "active"),
...
async def register_user(request: Request, db: Session = fastapi.Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            print(f"âš ï¸ [AUTH WARNING] Strategy seeding failed for {new_user.email}: {seed_err}")
            db.rollback()
        ...
        allowed = TokenCatalog.get_allowed_tokens(get_access_tier(new_user))
...
def read_my_entitlements(
    ...
    """
    DiagnÃ³stico de lÃ­mites y cuotas.
    """
...
async def read_users_me(current_user: User = fastapi.Depends(get_current_user)):
    ...
    allowed_tokens = TokenCatalog.get_allowed_tokens(get_access_tier(new_user))
