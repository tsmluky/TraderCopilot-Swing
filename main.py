...
def on_startup():
    ...
    if os.getenv("RUN_SCHEDULER", "false").lower() in ("1", "true", "yes"):
        ...
        t.start()
        LOG.warning("Scheduler thread started INSIDE API (RUN_SCHEDULER=true). Recommended: run scheduler.py as separate process.")
...
def compute_stats_summary(db: Session, user: User):
    ...
    # Open signals (Accurate): saved signals without evaluation row
